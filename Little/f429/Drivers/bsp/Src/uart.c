...
static uint8_t USART_TX_Buf[TX_BUF_LEN]; /* 发送缓冲区 */
...
void UART5_Init(uint32_t bound) {
    ...
    #if UART5_USE_DMA_TX
    /* 打开DMA发送 */
    UART5_TX_DMA_Init(2048, 128);
    #endif /* UART5_USE_DMA_TX */

    #if EN_UART5_RX
        #if UART5_USE_DMA_RX
    /* 使用DMA, 打开DMA接收 */
    UART5_RX_DMA_Init(4096, 128);
...
uint8_t g_usart6_rx_buf[RXBUFFERSIZE];
...
void USART6_IRQHandler(void) {
    ...
    if (__HAL_UART_GET_FLAG(&USART6_Handler, UART_FLAG_IDLE)) {
        __HAL_USART_CLEAR_IDLEFLAG(&USART6_Handler);
        UART_DMARX_Idle_Callback(&USART6_Handler);
    }
...
void USART6_Init(uint32_t bound) {
    ...
    /* 打开DMA发送 */
    USART6_TX_DMA_Init(2048, 128);
    #endif /* USART6_USE_DMA_TX */

    #if EN_USART6_RX
        #if USART6_USE_DMA_RX
    /* 使用DMA, 打开DMA接收 */
    USART6_RX_DMA_Init(4096, 128);
        #else  /* USART6_USE_DMA_RX */
    HAL_UART_Receive_IT(&USART6_Handler, (uint8_t *)g_usart6_rx_buf,
                        RXBUFFERSIZE);
        #endif /* USART6_USE_DMA_RX */
    #endif     /* EN_USART6_RX */
}
...
uint8_t g_uart7_rx_buf[RXBUFFERSIZE];
...
void UART7_Init(uint32_t bound) {
    ...
    #if UART7_USE_DMA_TX
    /* 打开DMA发送 */
    UART7_TX_DMA_Init(2048, 128);
    #endif /* UART7_USE_DMA_TX */

    #if EN_UART7_RX
        #if UART7_USE_DMA_RX
    /* 使用DMA, 打开DMA接收 */
    UART7_RX_DMA_Init(4096, 128);
        #else  /* UART7_USE_DMA_RX */
    HAL_UART_Receive_IT(&UART7_Handler, (uint8_t *)g_uart7_rx_buf,
                        RXBUFFERSIZE);
        #endif /* UART7_USE_DMA_RX */
    #endif     /* EN_UART7_RX */
}
...
uint8_t g_uart8_rx_buf[RXBUFFERSIZE];
...
void UART8_Init(uint32_t bound) {
    ...
    #if UART8_USE_DMA_TX
    /* 打开DMA发送 */
    UART8_TX_DMA_Init(2048, 128);
    #endif /* UART8_USE_DMA_TX */

    #if EN_UART8_RX
        #if UART8_USE_DMA_RX
    /* 使用DMA, 打开DMA接收 */
    UART8_RX_DMA_Init(4096, 128);
        #else  /* UART8_USE_DMA_RX */
    HAL_UART_Receive_IT(&UART8_Handler, (uint8_t *)g_uart8_rx_buf,
                        RXBUFFERSIZE);
        #endif /* UART8_USE_DMA_RX */
    #endif     /* EN_UART8_RX */
}
    ...
    if (huart->Instance == USART1) {
        ...
        if ((UART5_RX_STA & 0x8000) == 0) {
            ...
            if (UART5_RX_STA & 0x4000) {
                ...
                if (g_uart5_rx_buf[0] == 0x0d) {
                    ...
                    UART5_RX_BUF[UART5_RX_STA & 0X5FFF] = g_uart5_rx_buf[0];
                    UART5_RX_STA++;
        ...
        if ((USART6_RX_STA & 0x8000) == 0) {
            ...
            if (USART6_RX_STA & 0x4000) {
                ...
                /* 还没收到0x0d(即\r) */
                if (g_usart6_rx_buf[0] == 0x0d) {
                    USART6_RX_STA |= 0x4000;
                } else {
                    USART6_RX_BUF[USART6_RX_STA & 0X3FFF] = g_usart6_rx_buf[0];
                    USART6_RX_STA++;
        ...
        if ((UART7_RX_STA & 0x8000) == 0) {
            ...
            if (UART7_RX_STA & 0x4000) {
                ...
                /* 还没收到0x0d(即\r) */
                if (g_uart7_rx_buf[0] == 0x0d) {
                    UART7_RX_STA |= 0x4000;
                } else {
                    UART7_RX_BUF[UART7_RX_STA & 0X7FFF] = g_uart7_rx_buf[0];
                    UART7_RX_STA++;
        ...
        if ((UART8_RX_STA & 0x8000) == 0) {
            ...
            if (UART8_RX_STA & 0x4000) {
                ...
                /* 还没收到0x0d(即\r) */
                if (g_uart8_rx_buf[0] == 0x0d) {
                    UART8_RX_STA |= 0x4000;
                } else {
                    UART8_RX_BUF[UART8_RX_STA & 0X8FFF] = g_uart8_rx_buf[0];
                    UART8_RX_STA++;
...
void uart_print(UART_HandleTypeDef *huart, const char *__format, ...) {
    ...
    /* 往串口发送数据 */
    HAL_UART_Transmit(huart, (uint8_t *)&USART_TX_Buf, len, 0xFFFF);
}
